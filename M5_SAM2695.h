 ...
 */
#ifndef _M5_UNIT_MIDI_H
#define _M5_UNIT_MIDI_H

#include "M5_Synth_Def.h"
#include "Arduino.h"
...
#define MIDI_CMD_POLYPHONIC_AFTERTOUCH 0xA0  // Polyphonic Aftertouch (or Key Pressure)
#define MIDI_CMD_CONTROL_CHANGE \
    0xB0                                  // Control Change (or Channel Mode
                                          // Message)
#define MIDI_CMD_PROGRAM_CHANGE     0xC0  // Program Change
#define MIDI_CMD_CHANNEL_AFTERTOUCH 0xD0  // Channel Aftertouch (or Channel Pressure)
...
#define MIDI_CMD_TIMING_CLOCK       0xF8  // Timing Clock (used in System Real-Time Messages)
...
#define MIDI_CMD_ACTIVE_SENSING     0xFE  // Active Sensing (used in System Real-Time Messages)
...
class M5_SAM2695 {
   ...
   public:
    void begin(HardwareSerial *serial = &Serial1, int baud = MIDI_BAUD, uint8_t RX = 16, uint8_t TX = 17);
    ...
    void setReverb(uint8_t channel, uint8_t program, uint8_t level, uint8_t delayfeedback);

    void setChorus(uint8_t channel, uint8_t program, uint8_t level, uint8_t feedback, uint8_t chorusdelay);
    void setPan(uint8_t channel, uint8_t value);
    void setEqualizer(uint8_t channel, uint8_t lowband, uint8_t medlowband, uint8_t medhighband, uint8_t highband,
                      uint8_t lowfreq, uint8_t medlowfreq, uint8_t medhighfreq, uint8_t highfreq);

    void setTuning(uint8_t channel, uint8_t fine,
                   uint8_t coarse);  // 0-127, 64 is default t
    void setVibrate(uint8_t channel, uint8_t rate, uint8_t depth, uint8_t delay);

    void setTvf(uint8_t channel, uint8_t cutoff, uint8_t resonance);

    void setEnvelope(uint8_t channel, uint8_t attack, uint8_t decay, uint8_t release);

    void setModWheel(uint8_t channel, uint8_t pitch, uint8_t tvtcutoff, uint8_t amplitude, uint8_t rate,
